...
MIN_CRITICAL_FACT_CONFIDENCE = 0.70
MIN_FAMILY_MATCH_CONFIDENCE = 0.70

_FACT_LABELS = {
    "material": "材料",
    "dimensions": "主要尺寸",
    "tolerances": "公差",
    "manufacturing_family": "制造类型",
    "process_sequence": "流程表工序",
    "supported_drawing_type": "支持的图纸类型",
}

_EMPTY_FACT_VALUES = {"", "-", "未识别", "待确认", "unknown", "n/a", "none", "null"}

_DRAWING_QUANTITY_PATTERNS = (
    re.compile(r"\bQTY\s*[:=#]?\s*(\d{1,7})\b", re.IGNORECASE),
    re.compile(r"\bREQUIRED\s+QUANTITY\s*[:=#]?\s*(\d{1,7})\b", re.IGNORECASE),
    re.compile(r"(?:需求数量|数量|批量)\s*[:：=#]?\s*(\d{1,7})\s*(?:件|个|PCS?)?", re.IGNORECASE),
)


def _drawing_quantity(draft: ReviewDraftV2) -> tuple[int, float, list[str]] | None:
    """Return an explicitly labelled drawing quantity, never a guessed count."""

    candidates = [
        (item.requirement, item.confidence, item.evidence_ids)
        for item in draft.engineering_requirements
    ]
    candidates.extend(
        (item.value, item.confidence, item.evidence_ids)
        for item in draft.fields
        if item.value
    )
    candidates.extend(
        (item.text, 0.95, [item.id])
        for item in draft.evidence
        if item.text
    )
    for text, confidence, evidence_ids in candidates:
        for pattern in _DRAWING_QUANTITY_PATTERNS:
            match = pattern.search(text)
            if not match:
                continue
            quantity = int(match.group(1))
            if 1 <= quantity <= 1_000_000:
                return quantity, confidence, list(evidence_ids)
    return None


_PROCESS_OPERATION_KEYWORDS = (
    "进料", "上料", "滚压", "辊压", "弯曲", "折弯", "冲切", "冲裁", "检验",
    "喷漆", "喷涂", "表处", "贴膜", "包装", "装配",
)
_PROCESS_OPERATION_CODE = re.compile(r"\b(?:OP|INSP)\s*\d+\b", re.IGNORECASE)


def _process_document_operation_facts(draft: ReviewDraftV2) -> list[SourceFact]:
    """Keep evidence-grounded process rows as ordered downstream facts."""

    if draft.document_type != DocumentType.PROCESS_DOCUMENT:
        return []
    evidence_by_id = {item.id: item for item in draft.evidence}
    candidates: list[tuple[tuple[int, float], int, EngineeringRequirement]] = []
    allowed_categories = {"process_note", "inspection", "surface", "assembly", "other"}
    for source_index, requirement in enumerate(draft.engineering_requirements):
        text = requirement.requirement.strip()
        if (
            requirement.category not in allowed_categories
            or requirement.confidence < MIN_CRITICAL_FACT_CONFIDENCE
            or not requirement.evidence_ids
            or not (_PROCESS_OPERATION_CODE.search(text) or any(word in text for word in _PROCESS_OPERATION_KEYWORDS))
        ):
            continue
        positions = [
            (evidence.page, evidence.bbox[1])
            for evidence_id in requirement.evidence_ids
            if (evidence := evidence_by_id.get(evidence_id)) is not None and evidence.bbox is not None
        ]
        position = min(positions) if positions else (10_000, float(source_index))
        candidates.append((position, source_index, requirement))

    candidates.sort(key=lambda item: (item[0][0], item[0][1], item[1]))
    return [
        SourceFact(
            name=f"process_operation_{index:02d}",
            value=requirement.requirement.strip(),
            confidence=requirement.confidence,
            evidence_ids=list(requirement.evidence_ids),
        )
        for index, (_, _, requirement) in enumerate(candidates, start=1)
    ]
...
def build_drawing_facts(
    *,
    ...
    explicit_quantity = _drawing_quantity(draft)
    if explicit_quantity is not None:
        quantity, confidence, evidence_ids = explicit_quantity
        facts_by_name["quantity"] = SourceFact(
            name="quantity",
            value=str(quantity),
            confidence=confidence,
            evidence_ids=evidence_ids,
        )
    for process_fact in _process_document_operation_facts(draft):
        facts_by_name[process_fact.name] = process_fact
    corrections = human_field_corrections(rules=rules, decisions=decisions)
    ...
    missing.extend(["manufacturing_family", "machine_and_tooling_capability", "validated_geometry"])
    if "quantity" not in facts_by_name or not facts_by_name["quantity"].value.strip():
...
def assess_feature_inputs(
    *,
    feature: str,
    facts: DrawingFactsV1,
    profile: ClassroomReferenceProfile,
) -> FeatureInputAssessment:
    """Decide whether a route or amount may be shown without inventing inputs."""

    if feature not in {"process", "quote"}:
        raise ValueError("feature input assessment supports process or quote")

    fact_by_name = {item.name: item for item in facts.facts}
    missing: list[str] = []
    warnings: list[str] = []
    documented_process = [
        item
        for item in facts.facts
        if item.name.startswith("process_operation_")
        and item.value.strip()
        and item.confidence >= MIN_CRITICAL_FACT_CONFIDENCE
    ]
    supported_types = {
        DocumentType.MECHANICAL_DRAWING,
        DocumentType.ASSEMBLY_DRAWING,
        DocumentType.PROCESS_DOCUMENT,
    }
    if facts.document_type not in supported_types:
        missing.append("supported_drawing_type")
        warnings.append("当前文件不是可靠识别的机械零件图、装配图或工程流程表。")
    elif facts.document_type == DocumentType.PROCESS_DOCUMENT and not documented_process:
        missing.append("process_sequence")
        warnings.append("工程流程表未提取到带证据的工序行，不能据此生成路线或报价。")

    required_fact_names = ["dimensions"]
    if not (facts.document_type == DocumentType.PROCESS_DOCUMENT and documented_process):
        required_fact_names.insert(0, "material")
    for name in required_fact_names:
        fact = fact_by_name.get(name)
        value = (fact.value if fact else "").strip()
        if value.lower() in _EMPTY_FACT_VALUES or not fact or fact.confidence < MIN_CRITICAL_FACT_CONFIDENCE:
            missing.append(name)

    material = fact_by_name.get("material")
    material_value = (material.value if material else "").strip()
    material_reliable = bool(
        material
        and material_value.lower() not in _EMPTY_FACT_VALUES
        and material.confidence >= MIN_CRITICAL_FACT_CONFIDENCE
    )
    if not material_reliable and facts.document_type == DocumentType.PROCESS_DOCUMENT and documented_process:
        warnings.append(
            "流程表未给出可靠材料牌号；材料形态、净重和单价只使用明确标记的课堂假设。"
        )

    tolerance = fact_by_name.get("tolerances")
    tolerance_value = (tolerance.value if tolerance else "").strip()
    tolerance_reliable = bool(
        tolerance
        and tolerance_value.lower() not in _EMPTY_FACT_VALUES
        and tolerance.confidence >= MIN_CRITICAL_FACT_CONFIDENCE
    )
    if not tolerance_reliable:
        warnings.append("公差信息不完整，加工与成本结果只能作为课堂假设。")

    if profile.match_confidence < MIN_FAMILY_MATCH_CONFIDENCE:
        missing.append("manufacturing_family")
        warnings.append("图纸中没有足够信号可靠判断 CNC、钣金、注塑或装配类型。")

    missing = list(dict.fromkeys(missing))
    if missing:
        readable = "、".join(_FACT_LABELS.get(item, item) for item in missing)
        if feature == "quote":
            summary = f"缺少可靠的{readable}，未生成具体报价金额。"
            warnings.append("本次仅保留 AI 提取事实；补齐关键输入后再生成课堂参考报价。")
        else:
            summary = f"缺少可靠的{readable}，未套用通用 CNC 或其他工艺路线。"
            warnings.append("本次仅保留 AI 提取事实；确认制造类型和关键输入后再生成路线。")
        return FeatureInputAssessment(
            feature=feature,
            result_status="insufficient_input",
            summary=summary,
            missing_inputs=missing,
            warnings=list(dict.fromkeys(warnings)),
        )

    quantity = fact_by_name.get("quantity")
    quantity_is_explicit = bool(quantity and quantity.value.strip())
    critical_names = (
        ("dimensions",)
        if facts.document_type == DocumentType.PROCESS_DOCUMENT and documented_process
        else ("material", "dimensions")
    )
    critical_confidences = [fact_by_name[name].confidence for name in critical_names]
    if feature == "process" and quantity_is_explicit and tolerance_reliable and min(critical_confidences) >= 0.85 and profile.match_confidence >= 0.82:
        return FeatureInputAssessment(
            feature="process",
            result_status="ready",
            summary="图纸核心事实较完整，已生成可供工艺评审的参考路线。",
            warnings=["设备、工装、参数和检验方法仍需现场确认。"],
        )

    if not quantity_is_explicit:
        warnings.append("图纸未明确数量，本次使用版本化课堂批量假设。")
    if feature == "quote":
        warnings.append("材料单价、工时、费率和毛利率均为版本化课堂假设，不是实时询价。")
        summary = "核心图纸事实可用，具体金额基于带来源的课堂成本假设。"
    else:
        warnings.append("设备、检验和部分制造条件来自版本化课堂参考包。")
        summary = "核心图纸事实可用，路线包含需要现场确认的课堂假设。"
    return FeatureInputAssessment(
        feature=feature,
        result_status="assumptions_only",
        summary=summary,
        warnings=list(dict.fromkeys(warnings)),
    )


def _documented_operation_profile(text: str) -> tuple[str, str, list[str], list[str]]:
    """Return a conservative resource/tooling/check profile for a visible route row."""

    if any(keyword in text for keyword in ("滚压", "辊压")):
        return (
            "滚压/辊压成形线、在线切断设备",
            "成形辊组、导向与切断工装",
            ["截面成形", "长度与直线度", "表面压伤"],
            ["核对滚压后截面", "核对切断长度", "确认辊组与导向状态"],
        )
    if any(keyword in text for keyword in ("弯曲", "折弯")):
        return (
            "弯曲/折弯设备与定位工装",
            "弯曲模具、端部定位与防划伤工装",
            ["弯曲轮廓", "端部长度", "回弹与表面"],
            ["核对弯曲方向和顺序", "检查回弹与轮廓", "确认两端切断状态"],
        )
    if any(keyword in text for keyword in ("冲切", "冲裁", "预切", "精切", "翻边")):
        return (
            "冲床、冲切模具与端部定位工装",
            "冲切/翻边模具、定位防呆与废料排出",
            ["端部轮廓", "缺口与翻边", "毛刺和定位"],
            ["核对端部定位", "检查冲切轮廓与毛刺", "确认翻边方向和高度"],
        )
    if any(keyword in text for keyword in ("检验", "进料")):
        return (
            "来料/过程检验工位与适配量检具",
            "检验台、量具、限度样件与追溯记录",
            ["材料与规格", "外观与尺寸", "批次和证明文件"],
            ["按流程表项目逐项记录", "隔离不合格状态", "保留批次追溯"],
        )
    if any(keyword in text for keyword in ("喷漆", "喷涂", "表处")):
        return (
            "受控表面处理/喷涂设备",
            "清洗、遮蔽、挂具与喷涂工装",
            ["颜色与外观", "膜层/附着力", "遮蔽与挂点"],
            ["确认前处理和遮蔽", "检查外观与附着", "记录批次与返修"],
        )
    if any(keyword in text for keyword in ("贴膜", "包装")):
        return (
            "贴膜、标识与包装工位",
            "保护膜、标签、防错与包装防护",
            ["贴膜和标签", "数量与追溯", "储运防护"],
            ["确认外观清洁", "核对标签和数量", "检查包装防压伤"],
        )
    return (
        "流程表对应设备与工位",
        "对应工装、定位与防错设施",
        ["工序内容", "输入输出状态", "记录与追溯"],
        ["按流程表逐项确认", "记录异常与处置", "确认可进入下一工序"],
    )


def _build_documented_process_steps(
    *,
    facts: DrawingFactsV1,
    ...
    documented_facts = [
        item for item in facts.facts if item.name.startswith("process_operation_") and item.value.strip()
    ]
    steps: list[ProcessStepV2] = []
    for index, fact in enumerate(documented_facts, start=1):
        text = fact.value.strip()
        title, separator, detail = text.partition("：")
        if not separator:
            title, separator, detail = text.partition(":")
        operation = title.strip() if separator and len(title.strip()) <= 120 else text[:120]
        purpose = detail.strip() if separator and detail.strip() else text
        resource, tooling, characteristics, checks = _documented_operation_profile(operation)
        parameters = [
            ProcessParameterRequirement(
                name="工序参数",
                target="流程表未明确的速度、压力、节拍、模具间隙和检验频次待现场确认",
                source="pending",
        ...
        steps.append(
            ProcessStepV2(
                ...
                operation=operation,
                purpose=purpose,
                input_state="流程表所列来料或上一道已确认的合格状态",
                output_state=f"完成“{operation}”并形成可进入下一道流程的受控状态",
                equipment_capability=(
                    f"{resource}；现场已知能力：{equipment_note}"
                    if equipment_note
                    else f"{resource}；具体型号、吨位、精度和产能待现场确认"
                ),
                setup_and_datum="保留流程表顺序；定位、基准、方向和防错要求由现场工艺负责人确认",
                tooling_category=tooling,
                key_characteristics=characteristics,
                quality_checks=(
                    [*checks, f"现场检验条件：{inspection_note}"]
                    if inspection_note
                    else [*checks, "量具/检具能力、判定方法和抽样频次待确认"]
                ),
                parameters=parameters,
                source_fact_fields=[fact.name],
            )
        )
    return steps
...
def build_process_plan(
    *,
    ...
    steps = _build_documented_process_steps(
        facts=facts,
        request=request,
    ...
    documented_route = bool(steps)
    if not documented_route:
    ...
    if facts.source_status == "ai_extracted":
        risks.insert(
            ...
            ProcessRisk(
                code="PR-00",
                level="medium",
                concern="本次路线依据为 AI 提取事实，尚未由工程师逐项确认。",
                impact="识别偏差可能使材料、工序条件或检验要求与原文件不一致。",
                verification_action="对照原文件核对材料、尺寸、工序顺序和检验要求，再决定是否采用本路线。",
                owner_role="工艺负责人",
            ),
        )
    elif facts.review_business_status != "pass":
        upstream_status = {
    ...
    assumptions = [
        ...
        (
            "路线顺序复用工程流程表中带证据的工序行；设备、参数和控制点仍是待确认的课堂补充。"
            if documented_route
            else "路线由程序模板结合 AI 提取事实和公开参考资料生成；未自动生成机床程序、刀路或投产参数。"
            if facts.source_status == "ai_extracted"
    ...
    if facts.source_status == "ai_extracted":
        warnings.append("路线依据中的 AI 提取事实尚未由工程师逐项确认；不得直接投产。")
    elif facts.review_business_status != "pass":
        readable_status = {
            "needs_review": "仍有事项需要工程师确认",
            "blocked": "存在必须先处理的问题",
    ...
    if not values.get("material"):
        warnings.append(
            "材料牌号与厚度未形成可靠事实；当前路线和金额仅使用显式课堂假设，正式业务必须补齐。"
            if documented_route
            else "材料未形成可靠事实，工艺与报价必须暂缓。"
        )
    ...
    route_material = request.material_form or material
    inspection_strategy = [
    ...
    return ProcessPlanDraftV2(
        ...
        route_summary=(
            (
                f"按工程流程表中 {len(steps)} 道带证据工序形成的 {family_label} 路线草案，"
                f"批量暂按 {request.quantity} 件、材料形态暂按 {route_material}。路线保留源文件工序顺序；"
                "设备匹配、工装基准、材料牌号、节拍和投产参数仍须由现场负责人确认。"
            )
            if documented_route
            else (
