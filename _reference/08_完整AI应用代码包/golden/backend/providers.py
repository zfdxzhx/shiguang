...
PROMPT_VERSION = "drawing-review-v2.9-process-rows"
        ...
        content: list[dict] = [
            {
                ...
                "text": (
                    ...
                    "assembly and inspection notes. A visibly labelled order or batch quantity (QTY, QUANTITY, "
                    "PCS or 数量) must be a separate process_note requirement with evidence; never infer it from "
                    "hole counts. For a process_document, create one evidence-grounded requirement for every "
                    "visible INSP/OP operation row, preserving its code, name and listed substeps; include "
                    "inspection, surface-treatment and packaging rows instead of summarizing them away. "
                    "For every finding, separately state category, impact and "
        ...
        parts: list[dict] = [
            {
                "text": (
                    ...
                    "datums, surface/heat-treatment requirements, standards, assembly and inspection notes. A visibly "
                    "labelled order or batch quantity (QTY, QUANTITY, PCS or 数量) must be a separate process_note "
                    "requirement with evidence; never infer it from hole counts. For a process_document, create "
                    "one evidence-grounded requirement for every visible INSP/OP operation row, preserving its "
                    "code, name and listed substeps; include inspection, surface-treatment and packaging rows "
                    "instead of summarizing them away. Mark each "
...
class GeminiDeepSeekHybridProvider:
    ...
    def analyze(self, *, document: dict, page_paths: list[Path]) -> ProviderResult:
        ...
        try:
            ...
            # DeepSeek is an optional advisory stage. Its timeout, HTTP error,
            # malformed JSON, or schema failure must never discard the already
            # validated Gemini/K3 visual result. Only the safe exception type is
            # exposed; provider response bodies and credentials remain private.
            return ProviderResult(
                draft=visual.draft,
                metadata={
                    "provider": self.name,
                    ...
                    "prompt_version": PROMPT_VERSION,
                    "live_api": True,
                    "routing": f"{self.visual_provider_label} images → DeepSeek review failed safely; primary result retained",
                    "visual_stage": visual_stage,
                    "secondary_stage": {
                        "status": "failed_safely",
                        "provider": "deepseek",
...
class KimiVisionProvider:
    ...
    def _prompt(page_count: int) -> str:
        return (
            ...
            "dimensions into one requirement. A visibly labelled order or batch quantity (QTY, QUANTITY, PCS or 数量) "
            "must be a separate process_note requirement with evidence; never infer it from hole counts. For a "
            "process_document, create one evidence-grounded requirement for every visible INSP/OP operation row, "
            "preserving its code, name and listed substeps; include inspection, surface-treatment and packaging rows "
            "instead of summarizing them away. Every finding "
            "must include category, engineering impact and an "
