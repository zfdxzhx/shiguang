...
_DEFAULTS: dict[ManufacturingFamily, _ProfileDefaults] = {
    ...
    ManufacturingFamily.SHEET_METAL: _ProfileDefaults(
        quantity=100,
        material_form="金属板/卷料（课堂参考；实际牌号、板厚与表面状态待确认）",
...
def _infer_family(facts: DrawingFactsV1) -> tuple[ManufacturingFamily, float, list[str]]:
    ...
    keyword_groups = {
        ManufacturingFamily.SHEET_METAL: (
            "板厚", "钣金", "折弯", "弯曲", "冲压", "冲裁", "冲切", "滚压", "辊压",
            "翻边", "料卷", "卷料", "喷漆", "薄板", "sheet metal",
        ),
...
def _material_form(facts: DrawingFactsV1, family: ManufacturingFamily, fallback: str) -> str:
    ...
    process_signals = " ".join(
        item.value for item in facts.facts if item.name.startswith("process_operation_")
    )
    if (
        family == ManufacturingFamily.SHEET_METAL
        and any(keyword in process_signals for keyword in ("料卷", "卷料"))
        and (not material or "保护膜" in material)
    ):
        return "金属卷料（流程表可见；实际牌号、厚度与表面状态待确认）"
    if not material or material.lower() in {"未识别", "待确认", "unknown", "n/a", "none"}:
        if family == ManufacturingFamily.SHEET_METAL and any(
            keyword in process_signals for keyword in ("料卷", "卷料")
        ):
            return "金属卷料（流程表可见；实际牌号、厚度与表面状态待确认）"
        return fallback
    suffixes = {
...
def _quantity(facts: DrawingFactsV1, fallback: int) -> tuple[int, bool]:
    value = next((item.value.strip() for item in facts.facts if item.name == "quantity"), "")
...
def build_classroom_reference_profile(facts: DrawingFactsV1) -> ClassroomReferenceProfile:
    ...
    if family == ManufacturingFamily.SHEET_METAL and documented_process:
        equipment_capability = (
            "滚压/辊压成形线、弯曲设备、冲床与冲切/翻边模具、表面处理及检验包装工位；"
            "具体型号、吨位、模具状态、节拍和产能待现场确认"
        )
        inspection_capability = (
            "来料检验、过程检验、端部轮廓/弯曲检具和外观限度样件；"
            "量具、抽样频次与判定标准待现场确认"
        )
        special_requirements = (
            "保留流程表工序顺序；确认卷料牌号/厚度、保护膜、端部定位、翻边、喷漆与包装要求"
        )
    quantity, quantity_from_drawing = _quantity(facts, defaults.quantity)
    ...
    return ClassroomReferenceProfile(
        ...
        assumptions=[
            ...
            (
                "数量采用图纸明确标注；设备、工时、费率和成本为可编辑课堂假设，首次运行无需手工填写。"
                if quantity_from_drawing
                else "图纸未明确数量，数量、设备、工时、费率和成本均采用可编辑课堂假设；首次运行无需手工填写。"
            ),
