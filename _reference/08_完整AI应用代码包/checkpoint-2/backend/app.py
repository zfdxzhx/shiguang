"""Checkpoint 2 FastAPI app: independent AI review and direct PDF report."""
...
from .models import AnalysisRequest, ProviderConfigurationRequest, ReviewDraftV2
...
def create_application(
    ...
    *,
    ...
    app = FastAPI(title="Drawing AI Course Checkpoint 2", lifespan=lifespan)
    ...
    def health() -> dict:
        return {"ok": True, "mode": "local-only", "milestone": 2}
    ...
    @app.post("/api/v1/features/review/runs", status_code=202)
    def create_review_run(payload: AnalysisRequest) -> dict:
        run = service.create_analysis(payload.model_copy(update={"feature": "review"}))
        return service.feature_run_payload(run["id"])

    @app.get("/api/v1/features/history")
    def review_history(limit: int = 100) -> dict:
        return {"runs": service.feature_history(limit)}

    @app.get("/api/v1/features/runs/{analysis_id}")
    def review_run(analysis_id: str) -> dict:
        return service.feature_run_payload(analysis_id)

    @app.get("/api/v1/features/runs/{analysis_id}/report")
    def review_report(analysis_id: str):
        path, feature = service.create_feature_report(analysis_id)
        if feature != "review":
            raise ServiceError("only the review report is implemented in Checkpoint 2", 409)
        return FileResponse(path, media_type="application/pdf", filename="AI-drawing-review-report.pdf")
...
def backend_self_check(app: FastAPI) -> int:
    ...
    checks = [
        ...
        ("/api/v1/documents" in paths, "PDF 接入 API"),
        ("/api/v1/features/review/runs" in paths, "独立 AI 审核 API"),
        ("/api/v1/features/{feature}/runs" not in paths, "工艺和报价留作 CP3"),
    ]
