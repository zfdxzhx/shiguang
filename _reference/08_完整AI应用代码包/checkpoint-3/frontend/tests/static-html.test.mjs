...
test("static product bundle contains only the simple independent product flow", async () => {
  ...
  assert.doesNotMatch(bundle, /AI 运行模式|Mock \/ 回放|本地 OCR|授权本次 AI 处理|我确认该图纸可以发送给当前模型|选择授权后即可运行|确认工艺路线，进入预报价|完成本次人工复核|继续查看预报价|Google Gemini（可选扩展）|Kimi K3 high|可直接使用|result-top/);
});
