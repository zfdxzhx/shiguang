...
class AnnotatedDrawingPage(Flowable):
    ...
    def draw(self) -> None:
        ...
        for marker in self.markers:
            ...
            badge_width = max(
                2 * radius,
                pdfmetrics.stringWidth(badge_label, "Helvetica-Bold", 8.2) + 4 * mm,
            )
            half_badge_width = badge_width / 2
            badge_x = max(
                image_x + half_badge_width,
                min(left, image_x + self.image_width - half_badge_width),
            )
            badge_y = max(radius, min(bottom + height, self.image_height - radius))
            canvas.setFillColor(accent)
            if len(numbers) == 1:
                canvas.circle(badge_x, badge_y, radius, stroke=0, fill=1)
            else:
                canvas.roundRect(
                    badge_x - half_badge_width,
                    badge_y - radius,
                    badge_width,
                    2 * radius,
                    radius,
            ...
            canvas.setFont("Helvetica-Bold", 9.5 if len(numbers) == 1 else 8.2)
            canvas.drawCentredString(badge_x, badge_y - 3.2, badge_label)
        canvas.restoreState()
...
def build_review_report_pdf(
    ...
    *,
    ...
    report_stage_label = "AI 生成" if is_ai_report else "待确认版" if is_draft else "已确认版"
    document_title = f"{report_title}（{report_stage_label}）"
    ...
    def page_chrome(canvas, current_doc) -> None:
        canvas.saveState()
        if is_draft:
            canvas.setFillColor(colors.HexColor("#EEF1EF"))
    ...
    identity_rows = [
        ...
        [
            ...
            _paragraph("结果性质", label),
            _paragraph("AI 自动审核，直接生成参考报告" if is_ai_report else "AI 辅助发现，等待负责人确认", body),
        ],
    ]
    identity_spans: list[tuple] = []
    ...
    status_table = Table(
        [
            ...
            [
                ...
                _paragraph("AI 审核已完成" if is_ai_report else "等待负责人确认" if is_draft else "负责人已确认", status_value),
            ],
        ],
    ...
    conclusion = Table(
        [[_paragraph(_customer_friendly_text(review.get("conclusion") or draft.get("summary")), body, limit=900)]],
    ...
    if len(overview_rows) == 1:
        overview_rows.append([
            ...
            _paragraph("-", centered),
            _paragraph("AI 审核已完成" if is_ai_report else "等待负责人确认", centered),
        ])
    ...
    story.append(PageBreak())
    story.append(_paragraph("三、问题与处理建议", heading))
    story.append(_paragraph("以下为 AI 识别的问题、可能影响和建议；真实业务使用前请由专业人员核验。" if is_ai_report else "请逐项确认问题、可能影响、建议处理方式、负责人和当前进度。", small))
    story.append(Spacer(1, 2 * mm))
    detail_rows = [[
        _paragraph("编号", label),
        _paragraph("发现的问题", label),
        _paragraph("可能造成的影响", label),
        _paragraph("建议怎么处理", label),
        _paragraph("负责人/进度", label),
    ]]
    for index, issue in enumerate(issues, start=1):
        priority = "先解决" if issue.get("severity") == "blocked" else "需确认"
        ...
        detail_rows.append([
            _paragraph(f"#{index}\n{priority}", centered),
            _paragraph(_customer_friendly_text(issue.get("problem")), body, limit=900),
            _paragraph(_customer_friendly_text(issue.get("impact")), body, limit=900),
            _paragraph(_customer_friendly_text(issue.get("recommendation")), body, limit=900),
            _paragraph(
                f"{owner}\n"
                f"{'建议人工核验' if is_ai_report else human_labels.get(issue.get('human_decision'), issue.get('human_decision') or '需要负责人确认')}",
                small,
    ...
    if len(detail_rows) == 1:
        detail_rows.append([
            _paragraph("-", centered),
            _paragraph("未形成问题项。", body),
            _paragraph("-", centered),
            _paragraph("真实业务使用前请由专业人员核验。" if is_ai_report else "由负责工程师完成最终确认。", body),
            _paragraph("仅供参考" if is_ai_report else "需要确认", centered),
        ])
    detail_table = Table(
        detail_rows,
        colWidths=[14 * mm, 44 * mm, 41 * mm, 50 * mm, 27 * mm],
        repeatRows=1,
    )
    detail_style = [
