...
class AnalysisService:
    ...
    @staticmethod
    def _verification_state(settings: ProviderSettings, status: str | None = None) -> dict:
        ...
        return {
            ...
            "checked_at": utc_now() if current_status in {"verified", "failed"} else None,
        }
    ...
    def _record_provider_verification(self, settings: ProviderSettings, status: str) -> None:
        """Record only safe evidence for the currently active in-memory credentials."""

        if status not in {"verified", "failed"}:
            raise ValueError("provider verification status must be verified or failed")
        with self._provider_lock:
            ...
            if self._provider_settings != settings:
                return
            self._provider_verification = self._verification_state(settings, status)
    ...
    def feature_output(self, analysis_id: str) -> dict | None:
        analysis = self.db.get_analysis(analysis_id)
        if not analysis:
            raise ServiceError("analysis not found", 404)
        if analysis["technical_status"] != "completed":
        ...
        facts = DrawingFactsV1.model_validate(self._loads(stored["facts_json"]) or {})
        profile = build_classroom_reference_profile(facts)
    ...
    def create_feature_report(self, analysis_id: str) -> tuple[Path, str]:
        ...
        feature = str(analysis.get("feature") or "review")
        output = self.feature_output(analysis_id)
        if output is None:
