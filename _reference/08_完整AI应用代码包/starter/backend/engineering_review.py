...
def build_engineering_review(
    *,
    ...
    if blocker_count:
        disposition = "blocked"
        conclusion = (
            f"发现 {blocker_count} 项阻断问题和 {review_count} 项复核问题。"
            "补齐资料或修订图纸并重新审核前，不建议进入后续工程流转。"
        )
    elif review_count:
        disposition = "conditional"
        conclusion = (
            f"未发现程序性阻断，但仍有 {review_count} 项问题需要工程师逐项确认、修正或驳回。"
        )
    else:
        disposition = "ready_for_human_release"
        conclusion = "当前规则未发现阻断或复核项；仍需授权工程师对图纸和证据完成最终确认。"
    if report_stage == "draft":
        conclusion += " 本结论为 AI 与规则生成的审核草案，尚未形成正式工程决定。"
